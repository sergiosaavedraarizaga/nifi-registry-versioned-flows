...
bucketId=$(/opt/nifi-toolkit-1.16.3/bin/cli.sh registry list-buckets -u http://63.32.117.72:18080/ -ot json | jq '.[] | select(.name|test("dev")).identifier')
flows=$(/opt/nifi-toolkit-1.16.3/bin/cli.sh registry list-flows -b ${bucketId} -u http://63.32.117.72:18080/ -ot json | jq '.[].identifier')
...
do
  ...
  /opt/nifi-toolkit-1.16.3/bin/cli.sh nifi pg-import -b ${bucketId} -f ${flow} -fv 1 --registryClientId 4c2b988e-0183-1000-08cc-7bbc53445064 -u http://52.208.110.211:8443
done
